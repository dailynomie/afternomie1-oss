<script lang="ts">
   ...
   const saveLog = async (event)=>{
	...
	const response = await fetch('/start/addLog', {
			method: 'POST',
			body: JSON.stringify({ "uniqueid":event.detail,"lastrequest":lastrequest,"lastreset":lastvalidated,"expiredate":new Date(exp_date).getTime(),"expires":exp_checked, "confirmed":0 }),
			headers: {
