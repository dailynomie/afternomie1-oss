<script lang="ts">
    ...
    function decryptMessage(enckey:"DUMMY"){
        key = enckey
        var note = decryptObject(encnote,key)
    }
      ...
      var outcome = 0;
        ...
        if (noteLog.length == 0) {
          ...
          if (message.exp_checked && new Date(message.exp_date) < d) {
            statusmessage = "The access code for this note has been successfully verified. However, the creator of the note has set an expiration date for the note of "+message.exp_date+". This means that the note already expired and will not be revealed anymore."
            view = "expired"
            outcome = -1;
          }
          else {
          if (noteLog[0].LASTREQUEST >= noteLog[0].LASTRESET) {
           if (noteLog[0].LASTREQUEST == noteLog[0].LASTRESET) {
            ...
            var query = "UPDATE LOG SET LASTRESET = '" + newlastreset + "' WHERE UNIQUEID = '" + noteLog[0].UNIQUEID+"'";
             updateNoteLog(query)
              sendEmail();
            }
            if (noteLog[0].LASTRESET > d.setDate(d.getDate() - (message.inactivity))) {
            outcome = 1;
            statusmessage = "The access code for this note has been successfully verified. The QR code has been scanned "+ daydifference +" days ago for the first time. However, the defined period of time to wait before revealing the note has been defined by the creator of the note to "+message.inactivity.toString()+" days. This means that you still have to wait "+(message.inactivity-daydifference).toString()+" days before message will come available to you. Please return later to access the message."
            ...
            if (noteLog[0].LASTRESET > noteLog[0].LASTREQUEST) {
             outcome = 3
             var query = "UPDATE LOG SET LASTREQUEST = '" + noteLog[0].LASTRESET + "' WHERE UNIQUEID = '" + noteLog[0].UNIQUEID+"'";
             updateNoteLog(query)
             statusmessage = "The access code for this note has been successfully verified. The QR code has been scanned "+ daydifference +" days ago for the last time. However, the creator of this note has responded to the request after this date, meaning that the creator is still alive/consious and does not want to have the message revealed yet. So there is no need to worry. Thanks for making use of our service, make sure to store the QR code in a save place so you can use it next time when needed."
    ...
    async function sendEmail(){
      ...
      domain = "http://"+domain
        ...
        const response = await fetch('/emails/validateowner', {
			method: 'POST',
			body: JSON.stringify({ "domain":domain,"uniqueid":uniqueid,"primaryemail":message.email_primary,"secondaryemail":message.email_secondary,"subject":message.subject}),
			headers: {
				'content-type': 'application/json'
			}
		});
        ...
        <div class="pb-3">
            <div class="row">
              <div class="col-12 col-md-4 offset-md-4 mt-4">
                ...
                <input type="text" name="access_code" class={txtfieldclass} maxlength="1000" bind:value={key}>
            ...
            <div class="row pt-5 text-center">
              <div class="col">
                ...
                {:else}
                <button class="btn btn-primary btn-lg" on:click={()=>{accessNote()}}>Next</button>
                {/if}
              </div>
            </div>
        </div>
        ...
        <div class="pb-3">
          <div class="row">
            <div class="col-12 col-md-8 offset-md-2 mt-3 text-break">
              ...
              {/if}
              <p>Please save or print this note. Once you close this page this note will 
                become permanentely unaccessable in order prevent unauthorized access in the future.</p>
    ...
    <div class="row align-items-center min-content-height">
      <div class="col">
        ...
        <div class="pb-3">
          <div class="row">
            <div class="col-12 col-md-8 offset-md-2 mt-3 text-break">
             <p>Oops.... the note you are looking for is not accessable anymore.
              The most likely reasons are that 1) the creator has set an expiration date for accessing the note and 
              the note has expired for that reason or 2) that the note has been revealed previously and as a result further access was immediately blocked.
             </p>
            </div>
          </div>
        </div>
